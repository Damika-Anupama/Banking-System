...
describe('ManagerLoanApprovalComponent', () => {
  ...
  describe('paginating the approval queue', () => {
    beforeEach(() => {
      component.loans = Array.from({ length: 12 }, (_, i) => ({
        loan_basic_detail_id: `LN-${i + 1}`,
        customer_id: `CUS-${i + 1}`,
        amount: (i + 1) * 1000,
        interest: 12,
        duration_days: 180,
    ...
    it('pages five applications at a time', () => {
      expect(component.pagedLoans.length).toBe(5);
      expect(component.loanRangeStart).toBe(1);
      expect(component.loanRangeEnd).toBe(5);
      expect(component.totalLoanPages).toBe(3);
    });

    it('clamps the page to the valid range', () => {
      component.setLoanPage(99);
      expect(component.loanPage).toBe(3);
    ...
    it('shows the short last page correctly', () => {
      component.setLoanPage(3);

      expect(component.pagedLoans.length).toBe(2);
      expect(component.loanRangeStart).toBe(11);
      expect(component.loanRangeEnd).toBe(12);
    });

    it('starts a new search back at page 1', () => {
      component.setLoanPage(3);

      component.searchTerm = 'LN-1';
      component.onSearchChange();

      expect(component.loanPage).toBe(1);
    });

    it('keeps the visible page in range after a decision removes rows', () => {
      component.setLoanPage(3);

      // Deciding the last two loans leaves 10 rows = 2 pages; the getters
      // clamp so the manager is never left staring at an empty page 3.
      (component as any).removeFromList('LN-11');
      (component as any).removeFromList('LN-12');

      expect(component.totalLoanPages).toBe(2);
      expect(component.pagedLoans.length).toBe(5);
      expect(component.loanRangeStart).toBe(6);
    });

    it('reports 0–0 of 0 when the queue is empty', () => {
      component.loans = [];

      expect(component.loanCount).toBe(0);
