...
describe('ManagerSettingsComponent', () => {
  ...
  describe('template structure', () => {
    it('shows two layout-holding skeletons while loading, instead of a spinner', () => {
      component.isLoading = true;
      fixture.detectChanges();

      expect(fixture.nativeElement.querySelectorAll('app-skeleton-table').length).toBe(2);
      expect(fixture.nativeElement.querySelector('form')).toBeNull();
      expect(fixture.nativeElement.querySelector('.animate-spin')).toBeNull();
    });

    it('renders the demo page header and section cards once loaded', () => {
      component.isLoading = false;
      fixture.detectChanges();

      expect(fixture.nativeElement.querySelector('app-skeleton-table')).toBeNull();
      expect(fixture.nativeElement.querySelector('.demo-page-header')).not.toBeNull();
      expect(fixture.nativeElement.querySelectorAll('.demo-section-card').length).toBe(2);
      expect(fixture.nativeElement.querySelector('.glass-card')).toBeNull();
    });

    it('labels every field with the shared auth-field-label style', () => {
      fixture.detectChanges();

      const labels = fixture.nativeElement.querySelectorAll('label.auth-field-label');
      expect(labels.length).toBeGreaterThanOrEqual(7);
    });

    it('submits through the form so Enter triggers the same save path', () => {
