...
export class ManagerEmployeesComponent implements AfterViewInit, OnDestroy {
  ...
  /**
   * The roster resolves from local fixtures, so this only holds the skeleton
   * for a beat — but it keeps the loading behaviour identical to the pages
   * that really do fetch, instead of flash-swapping layouts.
   */
  isLoading = true;

  /** Mirrors the roster's headers so the loading skeleton holds the layout. */
  readonly rosterSkeletonColumns = ['Employee', 'Role', 'Joined', 'Transactions', 'Status', 'Actions'];
  ...
  rosterPage = 1;
  readonly rosterPageSize = 6;

  get rosterCount(): number {
    return this.filteredEmployees.length;
  }

  get totalRosterPages(): number {
    return Math.max(1, Math.ceil(this.rosterCount / this.rosterPageSize));
  }

  get pagedEmployees(): RosterEmployee[] {
    const page = Math.min(this.rosterPage, this.totalRosterPages);
    const start = (page - 1) * this.rosterPageSize;
    return this.filteredEmployees.slice(start, start + this.rosterPageSize);
  }

  get rosterRangeStart(): number {
    return this.rosterCount === 0
      ? 0
      : (Math.min(this.rosterPage, this.totalRosterPages) - 1) * this.rosterPageSize + 1;
  }

  get rosterRangeEnd(): number {
    return Math.min(this.rosterRangeStart + this.rosterPageSize - 1, this.rosterCount);
  }

  setRosterPage(page: number): void {
    this.rosterPage = Math.max(1, Math.min(page, this.totalRosterPages));
  }

  /** New search or role filter starts back at page 1 so matches are never hidden off-page. */
  onFilterChange(): void {
    this.rosterPage = 1;
  }
