...
export class TransactionComponent implements OnInit, OnDestroy {
  ...
  get filteredDecorated(): { row: any; index: number }[] {
    ...
    return this.txSort.apply(matched);
  }
  ...
  /**
   * Shared TableSort over the decorated rows, so a row keeps the ledger
   * position its reference is derived from even after reordering. The template
   * wires the header buttons straight to `txSort.toggle(...)` and resets the
   * page there — staying on page 3 of a re-sorted list shows rows nobody
   * asked for.
   */
  readonly txSort = new TableSort<{ row: any; index: number }>({
