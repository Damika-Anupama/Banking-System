...
export class FixedDepositComponent implements OnInit, OnDestroy {
  ...
  fdPage = 1;
  /** FD rows are compact two-line rows, so 3 per page keeps the card short without hiding data. */
  readonly fdPageSize = 3;

  get activeFdCount(): number {
    return this.fdCount;
  }

  get totalFdPages(): number {
    return Math.max(1, Math.ceil(this.activeFdCount / this.fdPageSize));
  }

  get pagedFds(): any[] {
    if (!Array.isArray(this.fds)) return [];
    const page = Math.min(this.fdPage, this.totalFdPages);
    const start = (page - 1) * this.fdPageSize;
    return this.fds.slice(start, start + this.fdPageSize);
  }

  get fdRangeStart(): number {
    return this.activeFdCount === 0 ? 0 : (Math.min(this.fdPage, this.totalFdPages) - 1) * this.fdPageSize + 1;
  }

  get fdRangeEnd(): number {
    return Math.min(this.fdRangeStart + this.fdPageSize - 1, this.activeFdCount);
  }

  setFdPage(page: number): void {
    this.fdPage = Math.max(1, Math.min(page, this.totalFdPages));
  }
