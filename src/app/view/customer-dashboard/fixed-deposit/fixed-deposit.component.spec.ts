...
describe('FixedDepositComponent', () => {
  ...
  describe('FD pagination', () => {
    it('pages fixed deposits three at a time', () => {
      component.fds = [{ fd_id: 1 }, { fd_id: 2 }, { fd_id: 3 }, { fd_id: 4 }];

      expect(component.activeFdCount).toBe(4);
      expect(component.totalFdPages).toBe(2);
      expect(component.pagedFds.length).toBe(3);

      component.setFdPage(2);

      expect(component.pagedFds.length).toBe(1);
      expect(component.fdRangeStart).toBe(4);
      expect(component.fdRangeEnd).toBe(4);
    });

    it('clamps out-of-range pages', () => {
      component.fds = [{ fd_id: 1 }];

      component.setFdPage(99);
      expect(component.fdPage).toBe(1);

      component.setFdPage(0);
      expect(component.fdPage).toBe(1);
    });

    it('reports an empty range when there are no deposits', () => {
      component.fds = [];

      expect(component.activeFdCount).toBe(0);
      expect(component.totalFdPages).toBe(1);
      expect(component.fdRangeStart).toBe(0);
      expect(component.fdRangeEnd).toBe(0);
      expect(component.pagedFds).toEqual([]);
    });
  });
});
