...
describe('HomeComponent', () => {
  ...
  describe('Linked accounts pagination', () => {
    const manyAccounts = Array.from({ length: 7 }, (_, i) => ({
      account_id: `ACC00${i + 1}`,
    ...
    beforeEach(() => {
      fixture = TestBed.createComponent(HomeComponent);
      component = fixture.componentInstance;
      component.accounts = [...manyAccounts];
    });

    it('pages accounts five at a time', () => {
      expect(component.filteredAccountCount).toBe(7);
      expect(component.totalAccountPages).toBe(2);
      expect(component.pagedAccounts.length).toBe(5);

      component.setAccountPage(2);

      expect(component.pagedAccounts.length).toBe(2);
      expect(component.accountRangeStart).toBe(6);
      expect(component.accountRangeEnd).toBe(7);
    });

    it('clamps out-of-range pages', () => {
      component.setAccountPage(99);
      expect(component.accountPage).toBe(2);

      component.setAccountPage(0);
      expect(component.accountPage).toBe(1);
    });

    it('reports an empty range when there are no accounts', () => {
      component.accounts = [];

      expect(component.filteredAccountCount).toBe(0);
      expect(component.totalAccountPages).toBe(1);
      expect(component.accountRangeStart).toBe(0);
      expect(component.accountRangeEnd).toBe(0);
      expect(component.pagedAccounts).toEqual([]);
    });

    it('paginates the filtered list, not the full one', () => {
      component.searchTerm = 'ACC001';

      expect(component.filteredAccountCount).toBe(1);
      expect(component.totalAccountPages).toBe(1);
      expect(component.pagedAccounts.length).toBe(1);
      expect(component.pagedAccounts[0].account_id).toBe('ACC001');
    });
    ...
    it('resets to page 1 when the search term changes', () => {
      ...
      component.searchTerm = 'branch';

      expect(component.accountPage).toBe(1);
    });

    it('resets to page 1 when a column sort toggles', () => {
      component.setAccountPage(2);

      component.toggleAccountSort('balance');

      expect(component.accountPage).toBe(1);
      expect(component.accountSort.stateFor('balance')).toBe('descending');
    });
  });
});
