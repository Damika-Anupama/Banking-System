...
describe('EmployeeHomeComponent', () => {
  ...
  describe('Rendered loading / error / table states', () => {
    ...
    it('shows the table skeleton while loading', () => {
      mockEmployeeHomeService.getEmployeeHome.and.returnValue(of({ data: [sampleCustomer] }));
    ...
    it('renders the customer table in a labelled region once loaded', () => {
      mockEmployeeHomeService.getEmployeeHome.and.returnValue(of({ data: [sampleCustomer] }));
      fixture.detectChanges();

      const shell = fixture.nativeElement.querySelector('.demo-table-shell');
      expect(shell).toBeTruthy();
      expect(shell.getAttribute('aria-label')).toBe('All customers');
      expect(fixture.nativeElement.querySelector('app-skeleton-table')).toBeFalsy();
    });

    it('shows a retry-able error card when loading fails', () => {
      mockEmployeeHomeService.getEmployeeHome.and.returnValue(throwError(() => new Error('boom')));
      fixture.detectChanges();

      const card = fixture.nativeElement.querySelector('.demo-state-card');
      expect(card).toBeTruthy();
      expect(card.textContent).toContain('boom');
      expect(fixture.nativeElement.querySelector('.demo-table-shell')).toBeFalsy();
    });

    it('shows an empty-state card inside the shell when no customers match', () => {
      mockEmployeeHomeService.getEmployeeHome.and.returnValue(of({ data: [sampleCustomer] }));
      fixture.detectChanges();
      ...
      expect(card).toBeTruthy();
      expect(card.textContent).toContain('No matching customers');
    });
  });
