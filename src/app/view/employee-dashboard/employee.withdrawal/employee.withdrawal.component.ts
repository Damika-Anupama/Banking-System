...
export class EmployeeWithdrawalComponent implements OnInit {
  ...
  /** True while the recent-withdrawal rows are being read into the view. */
  isLoading = true;

  /** Rough shape of the withdrawal table so the loading skeleton holds the layout. */
  readonly withdrawalSkeletonColumns = ['Withdrawal ID', 'Account', 'Amount', 'Time', 'Status'];
  ...
  get totalWithdrawalPages(): number {
    return Math.max(1, Math.ceil(this.withdrawals.length / this.withdrawalPageSize));
  }

  get pagedWithdrawals(): WithdrawalRecord[] {
    const page = Math.min(this.withdrawalPage, this.totalWithdrawalPages);
    const start = (page - 1) * this.withdrawalPageSize;
    return this.withdrawals.slice(start, start + this.withdrawalPageSize);
  }

  get withdrawalRangeStart(): number {
    return this.withdrawals.length === 0 ? 0 : (Math.min(this.withdrawalPage, this.totalWithdrawalPages) - 1) * this.withdrawalPageSize + 1;
  }

  get withdrawalRangeEnd(): number {
    return Math.min(this.withdrawalRangeStart + this.withdrawalPageSize - 1, this.withdrawals.length);
  }
  ...
  processWithdrawal(): void {
    ...
    Swal.fire({
      ...
      setTimeout(() => {
        ...
        // Jump back to the first page so the new withdrawal is visible.
