...
export class UnifiedDashboardComponent implements OnInit, OnDestroy {
  ...
  /**
   * Pages that are reachable but deliberately absent from the sidebar nav
   * (settings opens from the footer action buttons), keyed by final URL
   * segment. Without this the breadcrumb silently disappeared on them.
   */
  private readonly offNavPageLabels: Record<string, string> = {
    'settings': 'Settings',
   ...
   * Falls back to the off-nav page map (e.g. Settings) before giving up.
   */
  get currentPageLabel(): string {
    ...
    return this.offNavPageLabels[lastSegment] ?? '';
  }
