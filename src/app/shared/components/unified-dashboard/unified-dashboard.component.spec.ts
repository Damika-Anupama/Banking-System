...
describe('UnifiedDashboardComponent', () => {
  ...
  describe('navigation orientation', () => {
    ...
    it('labels the settings pages even though they have no sidebar nav entry', () => {
      // Settings opens from the footer action button, not the nav list, so the
      // nav-item match comes up empty; the off-nav fallback fills the crumb.
      atUrl('/dashboard/settings');
      expect(component.currentPageLabel).toBe('Settings');

      atUrl('/employee-dashboard/employee-settings');
      expect(component.currentPageLabel).toBe('Settings');

      atUrl('/manager-dashboard/manager-settings');
      expect(component.currentPageLabel).toBe('Settings');
    });
